...
cat >$RPM_BUILD_ROOT%{_scl_scripts}/register.d/60-post-fpm <<EOF
/bin/systemctl daemon-reload || :
EOF

cat >$RPM_BUILD_ROOT%{_scl_scripts}/deregister.d/40-preun-fpm <<EOF
/bin/systemctl --no-reload disable %{?scl_prefix}php-fpm.service || :
/bin/systemctl stop %{?scl_prefix}php-fpm.service  || :
EOF

cat >$RPM_BUILD_ROOT%{_scl_scripts}/deregister.d/60-postun-fpm <<EOF
/bin/systemctl daemon-reload || :
EOF
